...
const request_1 = require("../src/classes/request");
const fs = require("fs");
describe("test for request class", () => {
    let request;
    beforeEach(() => {
        request = new request_1.Request(new src_1.ClientTest("abc"));
    });
    it("Add headers", () => {
        request.addHeaders({
    ...
    it("Download test", async () => {
        ...
        const response = await request.download("https://httpbin.org/image/jpeg", savePath);
        expect(response.status).toBe(200);
